...
import os
from core.domain.pipeline import Pipeline, LoopStep
...
def build_pipeline_fun_fact() -> Pipeline:
    ...
    steps=[
      GenerateFunFactInputStep(
        ...
        lambda context: {
          ...
          "number": context["loop_index"] + 1,  # exibe 1, 2, 3...
        },
      ),
      GenerateImageStep(
      ...
      GenerateFunFactCanvas(
        ...
        lambda context: {
          ...
          "font_path": font_path,
          "number": context["generate_fun_fact_input_step"]["number"],
  ...
  # Pipeline principal com loop para n curiosidades
  return Pipeline(
    name="pipeline_fun_fact_country",
    description="Pipeline de geração de vídeo com curiosidade sobre um país",
    steps=[
      LoopStep(
        name="loop_n_facts",
        description="Executa a geração de n curiosidades",
        times=1,  # será sobrescrito por context["n"]
        step=single_fact_pipeline
      ),
